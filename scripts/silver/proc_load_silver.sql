	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN UPPER(TRIM(cst_gndr)) = 'M' THEN 'Male'
				 ...
				 ELSE 'n/a'
				 END AS cst_gner,  --Normalize gender values to readable format
			cst_create_date
		FROM (
		SELECT 
		* ,
		...
		SELECT 
			...
			CAST(LEAD(prd_start_dt) OVER(PARTITION BY prd_key ORDER BY prd_start_dt)-1 AS DATE) AS prd_end_dt_test -- Calculate end date as one day before the next start date(Data Enrichment)
		...
		SELECT
			...
			CASE WHEN sls_sales IS NULL OR sls_sales <= 0 OR sls_sales != sls_quantity * ABS(sls_price)
				 ...
				 ELSE sls_sales
				 END AS sls_sales, -- Recaluculate sales if original value is missing or incorrect
			sls_quantity,
